...
export class SQLiteDao implements IDao {
    ...
    async insertOrder(order: Array<Cart>) {
        ...
        try {
            ...
            console.log(await this.knex("carrito").join("ordenes", "carrito.orden_id", "=", "ordenes._id").join("productos", "carrito.producto_id", "=", "productos._id"));
            await this.knex.from('carrito').del();
            await this.knex.from('ordenes').del();
            // await knex.destroy();
        }
    }
